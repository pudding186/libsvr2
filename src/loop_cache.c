...
loop_ptr_queue* create_loop_ptr_queue(size_t size)
{
    loop_ptr_queue* lpq;
    char* ptr = (char*)malloc(sizeof(loop_ptr_queue) + sizeof(void*)*size);
    lpq = (loop_ptr_queue*)ptr;

    lpq->ptr_array = (void**)(ptr + sizeof(loop_ptr_queue));
...
void destroy_loop_ptr_queue(loop_ptr_queue* queue)
{
    if (queue)
    {
        free(queue);
...
bool loop_ptr_queue_push(loop_ptr_queue* queue, void* ptr)
{
    size_t dist = queue->tail + queue->size - queue->head;
    size_t used = dist >= queue->size ? (dist - queue->size) : dist;

    if (used >= queue->size - 1)
    {
        return false;
    }

    queue->ptr_array[queue->tail] = ptr;
...
void* loop_ptr_queue_pop(loop_ptr_queue* queue)
{
    size_t dist = queue->tail + queue->size - queue->head;
    size_t used = dist >= queue->size ? (dist - queue->size) : dist;
    ...
    void* ptr = queue->ptr_array[queue->head];
