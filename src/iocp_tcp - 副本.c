...
void _iocp_tcp_socket_connect_ex(HSESSION socket_ptr, BOOL reuse_addr)
{
    ...
    socket_ptr->socket = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
    ...
    //if (socket_ptr->local_addr_info)
    //{
    //    if (SOCKET_ERROR == 
    //        bind(socket_ptr->socket, 
    //             socket_ptr->local_addr_info->ai_addr,
    //             socket_ptr->local_addr_info->ai_addrlen))
    //    {
    //        goto ERROR_DEAL;
    //    }
    //}
    ...
    if (socket_ptr->local_sockaddr.sin6_family)
    {
        if (SOCKET_ERROR ==
            bind(socket_ptr->socket,
                ...
                sizeof(socket_ptr->local_sockaddr)))
        {
            goto ERROR_DEAL;
        }
    }
    ...
    if (!socket_ptr->mgr->func_connectex(
        ...
        sizeof(socket_ptr->peer_sockaddr),
...
bool _iocp_tcp_listener_listen(HLISTENER listener, unsigned int max_accept_ex_num, const char* ip, UINT16 port, bool bReUseAddr)
{
    ...
    listener->socket = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
...
bool _proc_net_event(HNETMANAGER mgr)
{
    ...
    case NET_EVENT_ESTABLISH:
    {
        ...
        mgr->func_on_establish(evt->evt.evt_establish.listener, evt->socket);
    }
    break;
    case NET_EVENT_MODULE_ERROR:
    {
        mgr->func_on_error(evt->socket, evt->evt.evt_module_error.err_code, 0);
    ...
    {
        mgr->func_on_error(evt->socket, error_system, evt->evt.evt_system_error.err_code);
        mgr->func_on_terminate(evt->socket);
    ...
    {
        mgr->func_on_error(evt->socket, error_connect_fail, evt->evt.evt_connect_fail.err_code);
...
bool _set_wsa_function(HNETMANAGER mgr)
{
    ...
    SOCKET tmp_sock = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
    ...
    const char* bind_ip, unsigned short bind_port)
{
    ...
    if (bind_ip)
    {
        ...
        {
            memcpy(&socket->local_sockaddr,
                &result->ai_addr,
                min(sizeof(socket->local_sockaddr), result->ai_addrlen));
            freeaddrinfo(result);
            result = 0;
        }
    }
